...
def on_connect(tag):
    ...
    if tag.ndef:
        ...
        # 새로운 NDEF 메시지 작성
        new_text_record = ndef.TextRecord("Hello,TokTokhan NFC!")
        new_uri_record = ndef.UriRecord('https://www.toktokhan.dev/')
        tag.ndef.records = [new_text_record,new_uri_record]
        print("새로운 NDEF 메시지가 작성되었습니다!")
    else:
